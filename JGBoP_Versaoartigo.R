...
jgbop.model<- sfcr_set(
  ...
  Lf ~ Lf[-1] - Savf,
  ##Banks
  Savb ~ (Rb - Rb[-1]) + (Bb - Bb[-1]) + (Lb - Lb[-1]) - (D - D[-1]),
  Piub ~ Savb,
  Pib ~ i * (Bb[-1] + Rb[-1] + Lb[-1] - D[-1]) ,
  Pidb ~ Pib - Piub,
  Lb ~ Lf,
  ##Gov
  Savg ~ -(B - B[-1]),
  ...
  ##RW - Fechamento
  CA ~ P * X - e * Ps * M - i * Bs[-1] ,
  Rcb ~ Rcb[-1] + ( (CA + (Bs - Bs[-1])) / e ) ,
  ...
  Bs ~  e * Ps * Ys * Phi * ( (1 + i) / ((1 + is) * (1 + phi_s) * (eet/e))),
  ee ~ eet[-1],
  eet ~ eet[-1] * (1 + pi) + varepsilon1 * (e[-1] - ee[-1]),
  
  er ~ e * Ps / P,
  
  e ~ eet[-1] * capt * epsilon * (1 - d(Bs) / Bs[-1]),
  # Here the exchange rate has a randon-walk component influenced by the CA.
...
jgbop.pars <- sfcr_set(
  ...
  Phi ~ calib.output$Phi,
  phi_d ~ calib.output$phi_d,
  varepsilon1 ~ calib.output$varepsilon1,
  epsilon ~ calib.output$epsilon,
  capt ~ sfcr_random("rnorm", 
                     mean = if (calib.output$CA < 0) 1.015 else 0.985,
                     sd = 0.025) ,
  #Capt stands for Current Account Pass-Through and its mean depends on the CA result.
