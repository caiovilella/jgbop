...
jgbop.calib <- sfcr_set(
  ...
  Rb ~ Bcb + e * Rcb, #Perguntar ao Italo
  ...
  Savf ~ -Lb * pi.fac,
  Savg ~ -B * pi.fac,
  CA ~ -Bs * pi.fac + e * Fs * (pis/ (1+pis)) + e * (Rcb * pis / (1 + pis)) , # What about Fs and Rcb?
  # CA ~ -Bs * pi.fac ,
  Savcb ~ Bcb * pi.fac - Rb * pi.fac - e * (Rcb * pis / (1+pis)),
  # Savcb ~ Bcb * pi.fac - Rb * pi.fac,
  Savb ~ -D * pi.fac + Lb * pi.fac + Bb * pi.fac + Rb * pi.fac + e * (Fs * pis / (1+pis)), # Savb is not zero and should not be. Counterfactually, it is negative in the model because of inflation-exchange rate nexus
  # Savb ~ -D * pi.fac + Lb * pi.fac + Bb * pi.fac + Rb * pi.fac,
  ...
  X ~ x * Y,
  M ~ m * Y,
  Ps ~ ( P * X - i * Bs / ( 1 + pi ) + is * e * Fs / (1+pis) + CA ) / ( e * M ), # This equation closes the RW sector
  ...
  Pidf ~ Pi - Piuf,
  SavhCheck ~ W * Nf + Pidf + Pidb - Ta + i * D / ( 1 + pi ) - P * C,
  Pib ~ i * ( Lb + Bb + Rb - D ) / ( 1 + pi ) + is * e * (Fs / (1+pis)), #this equation closes the banks' sector
  Piub ~ Savb,
  Pidb ~ Pib - Piub,
  YD ~ (1 - psi) * Y,
  Yk ~ K/(1+pi) / v,
  ...
  Phi2 ~ (Fs * (pis / (1+pis)) + Phi1 * D * ( ( 1 + i) / ((1 + is) * phi_d * (eet / e) )) ) / (e * pi.fac * (Fs / (1+pis))),
  eq ~ (i * Bs / (1+pi) - P * X) / (is * Fs / (1+pis) - Ps * M),
...
a <- a %>% mutate( teste5 = P * C + P * I + P * G + P * X - Ps * e * M - W * Nf - Pif - i * (Lb / (1+pi)) ) 
a$teste5
a <- a %>% mutate(testeF2 = SAVf + Lf * (pi / (1 + pi)))
a$testeF2
#Teste do Setor externo
a <- a %>% mutate( teste6 = CA  + Bs * (pi / (1+pi)) - e * (Rcb * pi / (1+pi)) - e * Fs * pi/(1+pi) )
a$teste6
a <- a %>% mutate( testeRW2 = CA - P * X + e * Ps * M - is * e * (Fs / (1+pi ) ) + i * (Bs / (1+pi)))
a$testeRW2
#Teste do BC
a <- a %>% mutate(testeBC = Picb - i * (Bcb / (1+pi)) + i * (Rb / (1+pi)))
a$testeBC
a <- a %>% mutate(testeBC2 = Rb * (pi / (1+ pi)) - Bcb * ( pi / (1+pi)) - e * (Rcb * (pi / (1+pi))))
a$testeBC2
