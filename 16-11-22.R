...
jgbop.model <- sfcr_set(
  ...
  Nf ~ min(N , Y * alpha) ,
  WBjg ~ wg * W * (N - Nf) ,
  YD ~ (1 - psi ) * (W * Nf + Pidf + Pidb + i * D[-1]) + WBjg ,
  C ~ (sigma1 * ((1 - psi ) * (W * Nf + i * D[-1]) +Pidf + Pidb) + sigma2 * Vh[-1] + sigma3 * WBjg) / P ,
  #crescimento
  u ~ Y / Yk,
  Yk ~ K[-1] / v ,
  I ~ delta * K[-1] ,
