...
jgbop.calib <- sfcr_set(
  ...
  Pif ~ Piuf / sf, 
  Picheck ~ P * ( C + I + G + X ) - Ps * e * M - i * Lb / ( 1 + pi ) - W * Nf, # This equation closes Firm's current sector
  Pidf ~ Pif - Piuf,
...
jgbop.model <- sfcr_set(
  ...
  # e ~ (P * X - i * B[-1] + (Bs -Bs[-1]) ) / ( Ps * M - is * Fs[-1] + (Rcb - Rcb[-1]) + (Fs - Fs[-1])),
  # e ~ ( Bs - Bs[-1]) / ((Fs - Fs[-1]) + (Rcb - Rcb[-1])),
  ...
  Fs ~  Fs[-1] + (D / P) / e * Phi1 * (((1 + is) * phi_d * (eet / e)) / (1 + i) ),
...
calib.output$Lb; tail(jgbop.base$Lb)
#Secao de testes
#teste horizontal
...
# O cenário é simulado com a função sfcr_scenario
...
#Cria coluna em cada data frame
wg_shock$scenario <- "Baseline"
wg_no_shock$scenario <- "Job Guarantee"
jg_scenario <- rbind(wg_shock, wg_no_shock)

#gera grafico comparando
jg_scenario %>%
ggplot( aes ( x= period, y = e, group = scenario, color = scenario, linetype = scenario) )+
  geom_line()

sfcr_dag_blocks_plot(jgbop.model, size=8)
